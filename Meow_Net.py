...
from Meow_Proliferate_and_Attention import Meow_Proliferate_and_Attention_Block

class Meow_Net(tf.keras.Model):
    
    def __init__(self, classes):
        super(Meow_Net, self).__init__()
        ...
        for i, args in enumerate(zip(self.dwkernels, self.strides, self.exps, self.outs, self.ratios, self.use_sa)):
            setattr(self, f"Meow_Proliferate_and_Attention_Block{i}", Meow_Proliferate_and_Attention_Block(*args))

    def call(self, inputs):
        x = self.relu(self.batchnorm1(self.conv1(inputs)))
        # Iterate through Meow Bottlenecks
        for i in range(16):
            x = getattr(self, f"Meow_Proliferate_and_Attention_Block{i}")(x)
